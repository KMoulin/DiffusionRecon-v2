function script_batch_DICOM_KM(struct_diff)
    ...
    if isfile([struct_diff.ReconFolder '/Trace.mat'])
        tmpMap=load([struct_diff.ReconFolder '/Trace.mat']);
        tmpEnum=tmpMap.enum;
        Recon_Local_Dicom_Maps(struct_diff,(tmpMap.Trace./nanmax(tmpMap.Trace(:)))*4096,tmpEnum,'Trace',10000,[0 4096])
    end
    
    if isfile([struct_diff.ReconFolder '/ADC.mat'])
        tmpMap=load([struct_diff.ReconFolder '/ADC.mat']);
    ...
    if isfile([struct_diff.ReconFolder '/DTI.mat'])
        EigValue=[];
        tmpMap=load([struct_diff.ReconFolder '/DTI.mat']);
    ...
    if isfile([struct_diff.ReconFolder '/HA2.mat']) && isfile([struct_diff.ReconFolder '/ROI.mat'])
        tmpMap=load([struct_diff.ReconFolder '/HA2.mat']);
        tmpMap2=load([struct_diff.ReconFolder '/ROI.mat']);
        Recon_Local_Dicom_Maps(struct_diff,(tmpMap.HA_filter4+90)*1e1.*tmpMap2.LV_Mask,tmpEnum,'HA',60000,[0 180*1e1]);
        Recon_Local_Dicom_Maps(struct_diff,(tmpMap.TRA2+90)*1e1.*tmpMap2.LV_Mask,tmpEnum,'TRA',70000,[0 180*1e1]);
        Recon_Local_Dicom_Maps(struct_diff,tmpMap.E2A*1e1,tmpEnum,'E2A',80000,[0 90*1e1]);
    end
     if isfile([struct_diff.ReconFolder '/HA2.mat']) && isfile([struct_diff.ReconFolder '/ROInet.mat'])
        tmpMap=load([struct_diff.ReconFolder '/HA2.mat']);
        tmpMap2=load([struct_diff.ReconFolder '/ROInet.mat']);
        Recon_Local_Dicom_Maps(struct_diff,(tmpMap.HA_filter4+90)*1e1.*tmpMap2.LV_Mask,tmpEnum,'HA',60000,[0 180*1e1]);
        Recon_Local_Dicom_Maps(struct_diff,(tmpMap.TRA2+90)*1e1.*tmpMap2.LV_Mask,tmpEnum,'TRA',70000,[0 180*1e1]);
        Recon_Local_Dicom_Maps(struct_diff,tmpMap.E2A*1e1,tmpEnum,'E2A',80000,[0 90*1e1]);
    end
...
%       for cpt_map=1:1:length(struct_diff.ListMaps)         
%           if (struct_diff.ListMaps(cpt_map).struct.istracew |  struct_diff.ListMaps(cpt_map).struct.istraceb0 | struct_diff.ListMaps(cpt_map).struct.isadc  | struct_diff.ListMaps(cpt_map).struct.isfa)
%             if length(struct_diff.ListMaps(cpt_map).struct.Listing)==size(Map,3)
%                 ListingMaps=struct_diff.ListMaps(cpt_map).struct.Listing;
%                 break;
%             end
%           end
%       end         
      %Batch_struct.struct_diff(1).Listing(1).folder
      uid = dicomuid;
      ...
      for cpt_slc=1:1:size(Map,3)
          ...
          for cpt_b=1:1:size(Map,4)
                   ...
                   tmpinfoDcm=dicominfo([struct_diff.Listing(1).folder  '\' struct_diff.Listing(1).name]);
                   ...
                   dicomwrite(uint16(tmpdataDcm),[struct_diff.ReconDcmFolder '/' MapName '_slc' num2str(cpt_slc) '_Bval' num2str(cpt_b) '_KM.dcm'], tmpinfoDcm, 'CreateMode', 'copy', 'WritePrivate' ,true); 
