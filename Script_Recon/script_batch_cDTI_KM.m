...
function script_batch_cDTI_KM(struct_diff,UI)
    ...
    if ~isempty(struct_diff.old_batch_path)
     ...
     enum.nset=1;
    save([enum.recon_dir '/RAW.mat'],'Dcm','enum');
    if UI.gif_mode
        mkdir([enum.recon_dir '/Gif/']);
        Gif_KM(Dcm, enum, 'Raw')
    end
    if length(enum.b)>1
    ...
    if UI.mosa_mode && enum.mosa>1
        ...
        save([enum.recon_dir '/Demosa.mat'],'Dcm','enum');
        if UI.gif_mode
    ...
    if UI.rigid_mode
        ...
        save([enum.recon_dir '/RigidBefore.mat'],'Dcm','enum');
        if UI.gif_mode
            Gif_KM(Dcm, enum, 'RigidReg_Before')
        end
    end

    if UI.Nrigid_mode
        [Dcm]= NonRigidRegistration_KM(Dcm, enum);
        save([enum.recon_dir '/NonRigid.mat'],'Dcm','enum');
        if UI.gif_mode
    ...
    if UI.pca_mode
        ...
        save([enum.recon_dir '/PCA.mat'],'Dcm','enum');
    end
    ...
    if UI.tmip_mode
        ...
        save([enum.recon_dir '/tMIP.mat'],'Dcm','enum');
         if UI.gif_mode
    ...
    if UI.avg_mode   
        ...
        save([enum.recon_dir '/Average.mat'],'Dcm','enum');
        if UI.gif_mode
    ...
    if UI.avg2_mode 
        ...
        save([enum.recon_dir '/Average_Reject.mat'],'Dcm','enum');
         if UI.gif_mode
    ...
    if UI.rigid_mode
        ...
        save([enum.recon_dir '/RigidAfter.mat'],'Dcm','enum');
        if UI.gif_mode
            Gif_KM(Dcm, enum, 'RigidReg_After')
        end
    end
    ...
    if UI.inter_mode
       ...
       [Dcm enum]=Scaling_KM(Dcm,enum,[200  1.9531]);
        save([enum.recon_dir '/Interpolation.mat'],'Dcm','enum');
         if UI.gif_mode
    ...
    if UI.trace_mode
        ...
        save([enum.recon_dir '/Trace.mat'],'Trace','Trace_Norm','enum');  
        if UI.gif_mode
    ...
    if UI.ADC_mode   
        ...
        save([enum.recon_dir '/ADC.mat'],'ADC');
    end
    ...
    if UI.mask_mode
        ...
        save([enum.recon_dir '/Mask.mat'],'Mask','Dcm');  
    end
    %%
    %%%%%%%%%%%%%%% Create ROI %%%%%%%%%
    if UI.roi_mode    
    ...
    if UI.DTI_mode
        ...
        save([enum.recon_dir '/DTI.mat'],'Tensor','EigValue','EigVector','MD','FA','Trace_DTI');  
       ...
       %%%%%%%%%%%%%% Extract HA %%%%%%%%%%%%%
        if UI.roi_mode
            ...
            [HA TRA E2A ]= HA_E2A_KM(EigVect1, EigVect2, LV_Mask, P_Epi, P_Endo);
            %[HA_filter]= HA_Filter_KM(HA,LV_Mask ,Mask_Depth,0);n

            [HA_filter]= HA_Filter_KM(HA,LV_Mask ,Mask_Depth,0);
            [HA_filter2]= HA_Filter_KM(HA_filter,LV_Mask ,Mask_Depth,0);
